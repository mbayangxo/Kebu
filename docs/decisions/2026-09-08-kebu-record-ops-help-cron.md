...
3. **Cron health** — `platform_cron_runs` written by billing-monthly · site-health · shop-subscriptions; failing sites from `site_health_checks`. Vercel Hobby only allows **once-per-day** cron expressions — keep `vercel.json` schedules daily (not `*/6`).
