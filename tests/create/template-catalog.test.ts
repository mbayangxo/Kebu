...
import { OWNER_PORTFOLIO_AESTHETIC_SLUGS } from "@/lib/create/user-aesthetics-catalog";
...
describe("template catalog", () => {
  ...
  it("hides owner portfolio brands from the public catalog", () => {
    for (const slug of OWNER_PORTFOLIO_AESTHETIC_SLUGS) {
      expect(TEMPLATE_SEEDS.some((t) => t.slug === slug)).toBe(true);
      expect(publicTemplateSeeds().some((t) => t.slug === slug)).toBe(false);
    }
    ...
    expect(publicTemplateSeeds().some((t) => t.slug === "scent-boutique")).toBe(true);
  });
