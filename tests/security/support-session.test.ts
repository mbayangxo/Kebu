...
describe("support session boundary", () => {
  it("binds a support session to both staff user and project", () => {
    const token = createSupportSessionToken({ userId: "staff-1", projectId: "project-1", sessionId: "session-1", role: "support", reason: "Ticket 1842", now: 1000 });
  ...
  it("expires after thirty minutes", () => {
    const token = createSupportSessionToken({ userId: "staff-1", projectId: "project-1", sessionId: "session-1", role: "support", reason: "Customer requested help", now: 1000 });
