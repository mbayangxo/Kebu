  ...
  return (
    ...
    <div className="min-h-screen" style={{ background: KEBU.bright }}>
      ...
      <div className="border-b border-black/10 bg-[#FFFCF8]"><div className="mx-auto max-w-[1440px] px-5 py-7 lg:px-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="mb-2 text-[10px] font-black uppercase tracking-[.28em]" style={{color:KEBU.orange}}>Kebu Studio</p><h1 className="text-3xl font-black tracking-[-.04em] lg:text-4xl">What will you create today?</h1><p className="mt-2 max-w-2xl text-sm text-black/55">Design, video, brand and campaign work in one place — with your Kebu business context already connected.</p></div><div className="flex gap-2"><Link href="/studio/templates" className="rounded-xl border border-black/10 bg-white px-4 py-2.5 text-xs font-bold">Browse themes</Link><Link href="/studio/new" className="rounded-xl bg-black px-4 py-2.5 text-xs font-black text-white">Create design <span style={{color:KEBU.orange}}>＋</span></Link></div></div>
      ...
      <main className="mx-auto max-w-[1440px] space-y-10 px-5 py-7 lg:px-8">
        ...
        {recentDesigns.length>0?<section><div className="mb-4 flex items-center justify-between"><h2 className="text-sm font-black">Continue creating</h2><span className="text-[11px] font-semibold text-black/40">{designs.length} designs</span></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">{designs.slice(0,6).map((d)=>{const fmt=FORMATS.find((f)=>f.type===d.design_type);return <Link key={d.id} href={`/studio/${d.id}`} className="group min-w-0"><div className="flex h-[150px] items-center justify-center rounded-2xl border border-black/10 bg-white group-hover:border-black/25"><CanvasThumb aspect={fmt?.aspect??1} accent={fmt?.accent??KEBU.orange}/></div><p className="mt-2 truncate text-xs font-bold">{d.title}</p><p className="mt-.5 text-[10px] capitalize text-black/40">{d.design_type.replace(/_/g," ")}</p></Link>})}</div></section>:null}
