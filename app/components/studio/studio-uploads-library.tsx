...
export function StudioUploadsLibrary({
  ...
  const filtered = useMemo(() => uploads.filter((item) => {
    ...
    return !q || (item.file_name ?? item.kind).toLowerCase().includes(q);
  }), [uploads, query, kind]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-1 overflow-x-auto">{(["all","image","video","audio"] as const).map((value)=><button key={value} type="button" onClick={()=>setKind(value)} className={`rounded-full px-2.5 py-1 text-[9px] font-bold capitalize ${kind===value?"bg-black text-white":"bg-black/[.04] text-black/55"}`}>{value}</button>)}</div>
      ...
      <ul className="grid max-h-[360px] grid-cols-2 gap-2 overflow-y-auto pr-1">{filtered.map((u)=><li key={u.id} className="group overflow-hidden rounded-xl border border-black/10 bg-white">
        <button type="button" disabled={readOnly || u.kind==="audio"} className="w-full text-left disabled:opacity-60" onClick={()=>u.kind==="video"?onPickVideo(u.url):u.kind==="image"?onPickImage(u.url):undefined}>
          ...
          <p className="truncate px-2 py-1.5 text-[9px] font-semibold">{u.file_name||u.kind}</p>
        </button>
