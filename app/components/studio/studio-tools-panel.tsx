...
type Action="brand"|"uploads"|"text"|"elements"|"layers";
type Tool={id:string;name:string;desc:string;icon:ComponentProps<typeof StudioIcon>["name"];action?:Action;href?:string};
const TOOLS:Tool[]=[
...
export function StudioToolsPanel({onAction,readOnly}:{onAction:(action:Action)=>void;readOnly?:boolean}){return <div className="space-y-3"><GalaxyPanelHeader eyebrow="Studio" title="Tools" description="Useful capabilities, without leaving your canvas."/><div className="grid grid-cols-2 gap-2">{TOOLS.map(t=>t.href?<a key={t.id} href={t.href} className="rounded-xl border border-black/10 bg-white p-3 hover:border-orange-400"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-black text-white"><StudioIcon name={t.icon} className="h-4 w-4"/></span><p className="mt-2 text-[10px] font-bold">{t.name}</p><p className="mt-0.5 text-[8px] leading-snug text-black/40">{t.desc}</p></a>:<button key={t.id} type="button" disabled={readOnly&&t.action==="text"} onClick={()=>t.action&&onAction(t.action)} className="rounded-xl border border-black/10 bg-white p-3 text-left hover:border-orange-400 disabled:opacity-40"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#FFF2E8]"><StudioIcon name={t.icon} className="h-4 w-4"/></span><p className="mt-2 text-[10px] font-bold">{t.name}</p><p className="mt-0.5 text-[8px] leading-snug text-black/40">{t.desc}</p></button>)}</div><p className="text-[9px] leading-relaxed text-black/35">Tools use the current Personal or Business Kebu space. More integrations can plug into this surface without changing the canvas model.</p></div>}